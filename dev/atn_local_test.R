...
# =====================================================================
# DRAFT from Cowork. Place in dev/ of the malariasimulation fork and run
# with the working directory = repo root (so the ./dev/ paths resolve).
#
# Division of labour: this script encodes the *experiment*. The four
# spots marked  ## VERIFY (Claude Code)  depend on the live fork API
# (override interface, rendering config, column names, set_bednets args)
# — hand to Claude Code to confirm/fix those against the real package,
# then run.
#
...
# ---------------------------------------------------------------------
# 3. Output rendering config   ## VERIFY (Claude Code)
# ---------------------------------------------------------------------
# malariasimulation only emits age-banded prevalence/incidence if asked.
# Confirm these parameter names AND the resulting column names (Section 4).
...
# Hill + Bompard: both should be TRUE by default; set explicitly until the
# get_parameters() default for use_bompard is confirmed TRUE (then delete).
...
run_arm <- function(arm, eir, res) {
  ...
  ## VERIFY (Claude Code): get_parameters overrides-list interface
  params <- get_parameters(overrides = c(render_overrides, form_overrides, atn_overrides))

  ## VERIFY (Claude Code): set_bednets arg name (timesteps) + coverage-0 future rounds OK
  params <- params |>
    set_bednets(
  ...
  ## VERIFY (Claude Code): run_simulation signature (can't pipe params in — timesteps is first)
  r <- run_simulation(timesteps = n_steps, parameters = params)

  ## VERIFY (Claude Code): exact rendered column names
  r |> as.data.frame() |>
    mutate(
