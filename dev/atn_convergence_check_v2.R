...
atn_df <- Map(function(dq, sl) {
  ...
  rt <- system.time(
    out <- run_arm("atn", eir, res, deltaq = dq, atn_window = atn_window, spor_len = sl)
  )[["elapsed"]]
  out$runtime_s <- rt
  out
...
# ---- 7. per-run compute time (s) over the deltaq x spor_len grid ----
...
p_rt <- ggplot(runtimes, aes(factor(deltaq, levels = deltaq_grid),
  ...
  coord_fixed() +
  labs(title = "Per-run compute time (s) — sequential",
